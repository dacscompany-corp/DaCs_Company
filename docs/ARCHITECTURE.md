...
| Quotations | quotation-module.js, quotation-print.js | Outgoing client quotations (0045) — itemized estimate, revisions, win/loss pipeline, print/PDF. **Deliberately isolated** like Reimbursement and Warranty Fund: no invoice, payment, expense, payroll or journal side effects, and no money math reads it. Marking a quote Won changes a status string and nothing else. Owner-only; staff never see it.<br>**The one sanctioned crossing is a FILE, not a query:** PM's Accomplishment Report builder can import a quotation **PDF the admin picks off disk** (`window.dacsParseQuotationPdf`, `js/print-utils.js`). Picking the file *is* the manual admin action the contract requires, and no code outside these two modules touches the tables. Never "improve" it into a quotation picker that queries `quotations` |
